...
async def get_dashboard_data(
    ...
    if start_date_str:
        ...
        registration_count = registration_count.where(
    ...
    if current_user.get("dept_id", "") == 3:
        ...
        user = await db_session.scalar(
            select(User).select_from(User).where(User.id == current_user.get("id"))
        )
        ...
        result.update({"sample_count": sample_count})
    elif current_user.get("dept_id", "") in (1, 2, 5, 6):
        sample_count = select(func.count()).select_from(Sample)
        ...
        weekly_registration_counts = select(
            cast(func.DATE_TRUNC("week", Registration.created_at), Date).label(
                "week_start"
            ),
        ...
        if start_date:
            weekly_registration_counts = weekly_registration_counts.where(
                Registration.created_at >= start_date
            )
        if end_date:
            weekly_registration_counts = weekly_registration_counts.where(
        ...
        weekly_registration_counts = await db_session.execute(
            weekly_registration_counts.group_by("week_start").order_by("week_start")
        )
        ...
        for row in weekly_registration_counts.all():
            ...
            result_dict = {
                ...
                "week": row[
                    0
                ],  # Assuming followup_count is an attribute of CustomerFollowUp
                # Add other fields from CustomerFollowUp and User as needed
                "count": row[1],
            }
            results.append(result_dict)

        result.update({"sample_count": sample_count, "registration_data": results})
...
@router.get("/sample/", response_model=SampleSchema)
async def get_sample_status(
    ...
    sample = await Sample.get_one(db_session, [Sample.sample_id == q])
    if sample is None:
